...
回头查日志，时间线解释了一切。node B 从 9 月 5 日到 9 月 16 日一直没重启，它的日志是完整的:

```
2026-09-15T21:53  两台热插线，链路 up            → 13 Gb/s
2026-09-15T22:02  node A 内核 Oops(见最后一节)
2026-09-15T22:15  cx7-pcie-hotplug: Cable removal   ← 线被拔掉
2026-09-15T22:17  node A 重启,四个口全报 Cable unplugged ← 这次重启时线是拔着的
2026-09-16T06:21  cx7-pcie-hotplug: Cable plugin     ← 再次热插
                  ...此后我测到的全部是 13 Gb/s
2026-09-16T09:54  node A 重启(线插着)
2026-09-16T09:55  node B 重启(线插着)
```
...
## 补记：线缆「自报规格对不上」是假象，ethtool 的锅

怀疑线的时候我把芯片信息读了出来，当时看哪一行都像实锤。后来逐字节对着规范查，**每一行都是老格式的解码假象**。
...
用 `sudo ethtool -m <网口> hex on` 把 256 字节原始数据读出来，逐条对:

| 看起来是 | 实际上 |
|---|---|
| `Identifier: 0x11 (QSFP28)` —— 一根老的 100G 级线 | SFF-8024 Rev 4.14 表 4-1:`11h` 的定义是「QSFP28 **或更新**,使用 SFF-8636 管理接口」。这个字节表示的是**管理接口/内存布局**,不是速率也不是形态。规范里**根本没有 QSFP112 这个标识符**;`1Eh` 表示改用了新一代的 CMIS。无源铜缆为兼容老主机普遍留在 SFF-8636,所以必须写 `11h` |
| `40G Base-CR4` —— 一根 40G 线 | byte 131 = `0x88`。低位 `0x08` 是「兼容 40GBASE-CR4」的声明;**高位 `0x80` 的含义是「真正的规格看 byte 192」**(SFF-8636 §6.3.4)。byte 192 = `0x3F` = **100GBASE-CR1 / 200GBASE-CR2 / 400GBASE-CR4**(802.3ck Clause 162,SFF-8024 Rev 4.14 表 4-4)。200G 那一档是下一个值 `40h`。所以这根线明确声明的是 100G/通道 那一族 |
| `BR, Nominal: 25500Mbps` —— 每通道才 25G | byte 140 = `0xFF`,是规范定义的**封顶值**,意思是「超过 25.4,去看 byte 222」。byte 222 = `0xD5` = 213 × 250 = **53.25 GBd**。SFF-8636 Rev 2.12 §6.3.6 专门把这两个字段从「BR, nominal / Mb/s」改名成「signaling rate, nominal / **MBd**」,原话就是为了避免 PAM4 产生混淆——**单位是波特不是比特**。配合 byte 139(PAM4,每符号 2 比特),每通道约 **106 Gb/s**,四通道约 426 |
| 40 厘米的线写着 `Length (Copper): 1m` | SFF-8636 §6.3.12:单位是米，而且明文规定「**不足 1 米的一律标 1 米**」。0.4 无法表示。小数级长度只有 CMIS 才有(byte 202,带 0.1 m 倍率) |
| 铜缆却有 `Length (OM1 62.5um): 7m` | 它根本不是长度。当 byte 147 高四位是 `1010b`(铜缆、未均衡，这里是 `0xa0`)时,byte 145 的含义是 **25.78 GHz 处的衰减,单位 dB** —— 也就是 7 dB。`ethtool` 不管三七二十一都按「OM1 米数」打印 |
| `Revision Compliance: Unallocated` —— 像是坏的 | byte 1 = `0x08` 表示 SFF-8636 Rev 2.8,而 `ethtool` 的对照表只到 `07h` |

**两个校验和也对得上**:用 bytes 128–190 和 192–222 重新计算，结果与芯片里存的 `0x2C`、`0x12` 一致。改过型号的贴牌线通常会把校验和算错。

**旁证**:NVIDIA 论坛上一份 2026 年 5 月的官方诊断附件里，另一根 Spark 用的 DAC(Mellanox `QSFP-200G-CU0.5M`,0.5 米)同样报 `Identifier 0x11`、同样 `BR 25500Mbps`、0.5 米的线同样写着 `Length (Copper): 1m`,区别只是它的 byte 192 是 `0x40`(200G 档)。一模一样的假象，线是好的。

**结论:光看 `ethtool -m` 判断不了一根 DAC 的速率等级。** 要看的顺序是:byte 131 的高位 → byte 192 → byte 222 配合 byte 139。

这一节的规范引用来自一次专门的资料核查(SFF-8024 Rev 4.14、SFF-8636 Rev 2.9/2.12),不是我本人通读规范得出的。

## 顺带发现的崩溃：mstflint 把内核搞崩了

查日志时发现，第一次插线之后九分钟,node A 内核 Oops:
...
- ~~线缆芯片信息与标称型号对不上~~ —— **已查清,见上一节「补记」**。本文原先写的是「是芯片信息写错了，还是贴牌，无法确认」,被逐字节对照 SFF-8636 / SFF-8024 规范推翻:芯片声明的是 400G-CR4 那一档、53.25 GBd、PAM4,校验和也对得上，一切正常。唯一真正没查清的是 `-RE` 后缀的含义(疑似经销商包装版本，未证实),以及这个型号在不同渠道被标成 40 cm 或 0.5 m —— 而 40 cm 和 0.5 m 在这个字段里都只能写成 `1`,所以从芯片里分辨不出来。
