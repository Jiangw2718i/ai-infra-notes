...
| 0.5 米的线写着 `Length (Copper): 1m` | SFF-8636 §6.3.12:单位是米，而且明文规定「**不足 1 米的一律标 1 米**」。0.5 无法表示。小数级长度只有 CMIS 才有(byte 202,带 0.1 m 倍率) |
...
- ~~线缆芯片信息与标称型号对不上~~ —— **已查清,见上一节「补记」**。本文原先写的是「是芯片信息写错了，还是贴牌，无法确认」,被逐字节对照 SFF-8636 / SFF-8024 规范推翻:芯片声明的是 400G-CR4 那一档、53.25 GBd、PAM4,校验和也对得上，一切正常。唯一真正没查清的是 `-RE` 后缀的含义(疑似经销商包装版本，未证实),以及这个型号在不同渠道被标成 40 cm 或 0.5 m(本机实物是 0.5 m,由购买者确认)—— 不足 1 米在这个字段里都只能写成 `1`。
